...
SdfPath
buildChildPath(UsdStageRefPtr stage,
...
bool
hasCompositionArc(const UsdPrim& prim)
{
...
TfTokenVector
insertChildOrderToken(const TfTokenVector& order, const TfToken& name, int index)
{
    TfTokenVector out;
...
bool
isInsideCompositionArc(UsdStageRefPtr stage, const SdfPath& path)
{
...
bool
movePrim(UsdStageRefPtr stage, const SdfPath& from, const SdfPath& toParent, QString& error)
{
    if (!stage) {
        error = "invalid stage";
        return false;
...
TfTokenVector
removeChildOrderToken(const TfTokenVector& order, const TfToken& name)
{
    TfTokenVector out;
...
QList<SdfPath>
leafPaths(UsdStageRefPtr stage, const QList<SdfPath>& mask, bool childMustBeWithinMask)
{
    QList<SdfPath> paths;
    if (!stage)
        return paths;
