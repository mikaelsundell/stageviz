...
materialUtilsApi()
{
    ...
    require(stageviz::MaterialUtils::shaderTypeLabel(previewIt->shaderId) == "UsdPreviewSurface", "shader type label");
  ...
  <standard_surface name="ImportedSurface" type="surfaceshader">
    ...
    const UsdShadeShader shader
        = stageviz::MaterialUtils::surfaceShader(UsdShadeMaterial(stage->GetPrimAtPath(imported.first())), &shaderId);
    ...
    require(GfIsClose(params.baseColor, GfVec3f(0.1f, 0.2f, 0.3f), 1e-5f) && closeEnough(params.metalness, 0.7, 1e-5)
                && closeEnough(params.roughness, 0.25, 1e-5) && closeEnough(params.ior, 1.7, 1e-5),
            "MaterialX imported parameters roundtrip");
}
